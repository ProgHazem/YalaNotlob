class OrdermembersController < ApplicationController
  ...
  def join_member
    respond_to do |format|
      @ordermember = Ordermember.find(params[:member_id])
      if @ordermember.update_attributes(:status => 'joined')
        format.html { redirect_to user_order_path(@ordermember.order.user,@ordermember.order), notice: 'You have successfully joined.' }
